...
public:
  ...
  GridManipulator& operator=(const GridManipulator&) = delete;
	...
	static constexpr tuple<bool, uint16_t> get_zero_count(unsigned char grid[])
	{
		...
		{
			if (grid_safety_check(grid) == false) return { false, 0 };
		}
	...
	template<bool do_safety_check = false>
	static bool update(unsigned char grid[], std::initializer_list<const uint8_t> operations)
	{
		if constexpr (do_safety_check)
		{
			bool ok = true;
	...
	{
		const uint16_t * const girdHelper = reinterpret_cast<const uint16_t* const>(grid);
		if constexpr (with_line_no) cout << endl << "\t0123456789abcdef" << endl << endl;
		for (int i = 0; i < LEN; i++)
		{
			if constexpr (with_line_no) cout << dec << i << "\t";
...
private:
	...
	static constexpr uint8_t get_col(const uint8_t addr)
	{
		return addr & 0x0f;
	}
	...
	static inline void update_grid(uint16_t * const girdHelper, const uint8_t row, const uint8_t col) noexcept
	{
	...
	static inline void update_row_col(uint16_t * const girdHelper, const uint8_t row, const uint8_t col) noexcept
	{
	...
	static inline bool ops_safety_check(const std::initializer_list<const uint8_t>& operations) noexcept
	{
		// TODO
		// suppress unused warning
		(void)operations;
